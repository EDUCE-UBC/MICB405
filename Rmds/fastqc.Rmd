...
Look at the `help` page for `fastp` and choose one trimming or filtering step. Apply this filtering to the Ebola fastqs and output the filtered fastq to a folder in your home directory. When you run `fastqc` on these filtered fastqs, can you observe the difference in the report stemming from the filtering steps you enacted? You can also use https://fastq.sandbox.bio/ to play around with different `fastp` parameters!
