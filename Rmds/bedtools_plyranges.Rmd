...
If you do not have these installed on your machine already, you will encounter and error, and will have to run their install commands:
```{r eval = FALSE}
...
Download the following files from the Orca server:
```{bash eval = FALSE}
...
These are the `MACS2` narrowPeak BED file from that tutorial, as well as a GTF (i.e. GFF3)-formatted file containing mouse gene information, which have been stripped of non-autosomal and non-canonical chromosomes for compatibility (different patches of mm10 will have different names for mitochondria and different "alternate" chromosomes, which makes `plyranges` angry).

## Genome arithmetic

1. Let's start simple: what is a command you can run to display what the unique seqnames (i.e. chromosomes) of each file are? Remember that these are annotations on a mouse genome!
...
1. Unfortunately, a left join means that some of your peaks will not have any genome information attached to it, as it did not overlap anything. What is a command that you could run to annotate the nearest genome information onto your peaks?
